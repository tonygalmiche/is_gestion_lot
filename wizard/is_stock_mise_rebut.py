        ...
        if data:
            ...
            if data.operation in ('bloque', 'debloque', 'change_location'):
                ...
                if data.operation == 'bloque':
                    action_model,action_id = data_pool.get_object_reference(cr, uid, 'is_gestion_lot', "action_stock_lot_bloque_report")
                    action_pool = self.pool.get(action_model)
                    action = action_pool.read(cr, uid, action_id, context=context)
                    return action             
                elif data.operation == 'debloque':
                    action_model,action_id = data_pool.get_object_reference(cr, uid, 'is_gestion_lot', "action_stock_lot_debloque_report")
                    action_pool = self.pool.get(action_model)
                    action = action_pool.read(cr, uid, action_id, context=context)
                    return action
                elif data.operation == 'change_location':
                    action_model,action_id = data_pool.get_object_reference(cr, uid, 'is_gestion_lot', "action_stock_lot_change_location_report")
                    action_pool = self.pool.get(action_model)
                    action = action_pool.read(cr, uid, action_id, context=context)
                    return action
                else:
                ...
                action_model,action_id = data_pool.get_object_reference(cr, uid, 'is_gestion_lot', "action_stock_lot_rebut_report")
                action_pool = self.pool.get(action_model)
                action = action_pool.read(cr, uid, action_id, context=context)
                return action
